...
function expStart(subject, session) {
  ...
  jsPsych.init({
    ...
    on_close: function() {
      jsPsych.data.get().localSave("csv", `NBack_Subject_${subjectId}_Session_${sessionNumber}_${level}back_quitted_output.csv`);
    },
    on_finish: function() {
      jsPsych.data.get().localSave("csv", `NBack_Subject_${subjectId}_Session_${sessionNumber}_${level}back_output.csv`);
    }
  });
